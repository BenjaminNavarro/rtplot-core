...
RTPlotCore::Pairf RTPlotCore::getToggleButtonPosition() {
	return Pairf{getXPosition()+getWidth()-_plot_margin_right + 10, getYPosition() + 10};
}
...
void RTPlotCore::drawPlot() {
	...
	plot_size_ = std::make_pair(getWidth() - _plot_margin_left - _plot_margin_right - label_area_width_, getHeight() - _plot_margin_top - _plot_margin_bottom);
	...
	if(display_cursor_coordinates_) {
		...
		drawText(std::to_string(p.first) + ", " + std::to_string(p.second), PointXY{getXPosition()+10, getYPosition()+getHeight()-10});
	}
