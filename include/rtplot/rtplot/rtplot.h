 ...
 * RTPlot can handle multiple plots inside the same window, each containing multiple curves, in real time with minimum CPU and memory usage.
 */
class RTPlot
{
public:
	 ...
	 * @param plot  the index of the plot containing the curve. Must be in the [0, \a rows*\a cols[ interval.
	 ...
	 * @param plot  the index of the plot containing the curve. Must be in the [0, \a rows*\a cols[ interval.
	 * @param curve the index of the curve. Must match with the index of a previously created curve (see addPoint).
	 */
	...
	*/
	...
	/**
	 * Set the label for a given curve. Will be displayed inside the curves' label area on the right of the plot.
	 * @param plot  the index of the plot containing the curve. Must be in the [0, \a rows*\a cols[ interval.
	 * @param curve the index of the curve. User defined, can be any number.
	...
	*/
	...
	/**
	 * Set the maximum number of points to be displayed for a given plot. The same value is applied to all curves.
	 * @param plot the index of the plot containing the curve. Must be in the [0, \a rows*\a cols[ interval.
	...
	/**
	 * Start plotting the data and respond to events triggered by the user. This is a blocking call, the function will return once the window is closed.
	 */
	virtual void run() = 0;

	/**
	* Refresh the window if necessary and handle events triggered by the user. This is a non-blocking call.
	* return true if the window is still alive, false otherwise.
	*/
